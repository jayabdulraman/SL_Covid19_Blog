...
{% set KPI_CASE = KPI_CASE or 'Africa' %}
{% set KPIS_INFO = KPIS_INFO or [{'title': 'Sierra Leone', 'prefix': 'SL'}, {'title': 'Guinea', 'prefix': 'GU'}, {'title': 'Liberia', 'prefix': 'LI'}, {'title': 'South Africa', 'prefix': 'SA'}] %}
...
{% set TOPLINKS = TOPLINKS or [
  {'title': 'Africa', 'href': '../covid-overview-af/'}] %}
...
{% if KPI_CASE == 'Africa' %}
  ...
  Of which <b class="color-neg">{{ '{0:,.0f}'.format(D['SA Cases (+)']) }}</b> ({{ "{0:.0%}".format(D['SA Cases (+)'] / D['Cases (+)']) }}) are from <b>South Africa</b>. South Africa currently <b>top</b> the chart in Africa in Confirmed Cases.
  Also, in the last <b>{{ lastdays }} days</b>, <b class="color-neg">{{ '{0:,.0f}'.format(D['SL Cases (+)']) }}</b> new Coronavirus cases in <b>Sierra Leone</b>.
  <b>Guinea</b> has reported <b class="color-neg">{{ '{0:,.0f}'.format(D['GU Cases (+)']) }}</b> new cases in the last {{ lastdays }} days.
  <b>Liberia</b> has reported <b class="color-neg">{{ '{0:,.0f}'.format(D['LI Cases (+)']) }}</b> new Coronavirus cases
  in the last <b>{{ lastdays }} days</b>.
{% elif KPI_CASE == 'Sierra Leone' %}
  In the last <b>{{ lastdays }} days</b>, <b class="color-neg">{{ '{0:,.0f}'.format(D['Cases (+)']) }}</b> new Coronavirus cases in Sierra Leone.
{% elif KPI_CASE == 'South Africa' %}
  In the last <b>{{ lastdays }} days</b>, <b class="color-neg">{{ '{0:,.0f}'.format(D['Cases (+)']) }}</b> new Coronavirus cases have been reported in South Africa.
{% elif KPI_CASE == 'Guinea' %}
  In the last <b>{{ lastdays }} days</b>, <b class="color-neg">{{ '{0:,.0f}'.format(D['Cases (+)']) }}</b> new Coronavirus cases have been reported in Guinea.
{% elif KPI_CASE == 'Liberia' %}
  In the last <b>{{ lastdays }} days</b>, <b class="color-neg">{{ '{0:,.0f}'.format(D['Cases (+)']) }}</b> new Coronavirus cases have been reported in Liberia.
{% else %}
  ''
{% endif %}
{%- endmacro %}
  ...
  <table class="table" style="width:575px;">
    ...
    {% for index, row in table.iterrows() %}
      <tr>
        ...
        <td class="pl1">{{ '{0:,.0f}'.format(row['Recoveries']) }}</td>
        <td class="change neg">(<b>{{ '{0:+,.0f}'.format(row['Recoveries (+)']) }}</b>)</td>
